...
import com.docsinclouds.glucose2db.G5Utils.ForegroundServiceStarter;
...
public abstract class G5BaseService extends Service {
    ...
    private final PowerManager.WakeLock wl = HelperClass.getWakeLock("g5-base-bt", 100);
    ...
    public void onCreate() {
        super.onCreate();
        service = this;
        Log.d("FOREGROUND", "Current Service: " + service.getClass().getSimpleName());
        startInForeground();
    }
    ...
    protected synchronized void extendWakeLock(long ms) {
        HelperClass.releaseWakeLock(wl); // lets not get too messy
        wl.acquire(ms);
    }
    ...
    protected void checkPreferenceKey(final String key, final SharedPreferences prefs) {
        if (key.equals("run_service_in_foreground")) {
            ...
            if (prefs.getBoolean("run_service_in_foreground", false)) {
                startInForeground();
                Log.i(service.getClass().getSimpleName(), "Moving to foreground");
            } else {
                stopInForeground();
                Log.i(service.getClass().getSimpleName(), "Removing from foreground");
            }
        }
    }
    ...
    protected void stopInForeground() {
        ...
        if (service != null) {
            ...
            Log.e("FOREGROUND", "Cannot stop foreground as service is null");
        }
        foregroundStatus();
    }

    protected void foregroundStatus() {
        Inevitable.task("foreground-status", 2000, () -> Log.d("FOREGROUND", service.getClass().getSimpleName() + (HelperClass.isServiceRunningInForeground(service.getClass()) ? " is running in foreground" : " is not running in foreground")));
    }

    protected static byte[] nn(final byte[] array) {
        if (array == null) {
            if (HelperClass.ratelimit("never-null", 60)) {
                Log.wtf("NeverNullG5Base", "Attempt to pass null!!! " + HelperClass.backTrace());
                return new byte[1];
            }
        }
        return array;
