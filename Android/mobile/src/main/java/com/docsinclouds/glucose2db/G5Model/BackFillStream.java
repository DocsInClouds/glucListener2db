...
import static com.docsinclouds.glucose2db.G5Model.DexTimeKeeper.fromDexTimeCached;
...
public class BackFillStream extends BaseMessage {
        ...
        if (this_sequence == last_sequence + 1) {
            ...
            for (int i = 2; i < packet.length; i++) {
                if (data.position() < data.limit()) {
                    ...
                    Log.wtf(TAG, "Reached limit for backfill stream size");
                }
            }
        } else {
            Log.wtf(TAG, "Received backfill packet out of sequence: " + this_sequence + " vs " + (last_sequence + 1));
        }
    }
    ...
    public List<Backsie> decode() {
        ...
        while (data.position() < extent) {
            ...
            switch (state) {
                ...
                default:
                    Log.wtf(TAG, "Encountered backfill data we don't recognise: " + type + " " + glucose + " " + trend + " " + " " + HelperClass.dateTimeText(fromDexTimeCached(dexTime)));
                    break;
    ...
    public void enumerate(int size) {
        ...
        while (i < data.position()) {
            ...
            System.out.print(HelperClass.bytesToHex(new byte[]{output[i]}));
            i++;
        }
        System.out.println("\n");
    }
