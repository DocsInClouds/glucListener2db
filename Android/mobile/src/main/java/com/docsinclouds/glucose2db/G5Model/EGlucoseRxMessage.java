...
public class EGlucoseRxMessage extends BaseGlucoseRxMessage {
    ...
    public EGlucoseRxMessage(byte[] packet) {
        Log.d(TAG, "EGlucoseRX dbg: " + HelperClass.bytesToHex(packet));
        if (packet.length >= 14) {
            ...
            if ((data.get() == opcode) && checkCRC(packet)) {
                ...
                Log.d(TAG, "EGlucoseRX: seq:" + sequence + " ts:" + timestamp + " sg:" + glucose + " psg: " + predicted_glucose + " do:" + glucoseIsDisplayOnly + " ss:" + status + " sr:" + status_raw + " st:" + CalibrationState.parse(state) + " tr:" + getTrend());
