...
import com.docsinclouds.glucose2db.G5Utils.SensorSendQueue;
...
public class Sensor extends Model {
  ...
  public String sensor_location;
    ...
    public synchronized static void stopSensor() {
        ...
        sensor.stopped_at = HelperClass.tsl();
        Log.d("SENSOR", "Sensor stopped at " + HelperClass.dateTimeText(sensor.stopped_at));
        sensor.save();
        if (currentSensor() != null) {
            Log.wtf(TAG, "Failed to update sensor stop in database");
        }
        SensorSendQueue.addToQueue(sensor);
        HelperClass.clearCache();
    ...
    public static boolean stoppedRecently() {
        ...
        return last != null && last.stopped_at < HelperClass.tsl() && (HelperClass.msSince(last.stopped_at) < (
...
//            Home.toaststaticnext("Error on Sensor sync."); //DIC
            return null;
        }
    }
