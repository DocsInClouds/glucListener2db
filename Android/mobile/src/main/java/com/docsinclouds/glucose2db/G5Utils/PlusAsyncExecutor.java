...
public class PlusAsyncExecutor implements Executor {
    ...
    public synchronized void execute(@NonNull final Runnable r) {
        ...
        final String queueId = HelperClass.backTraceShort(0); // TODO probably a better way to get a queue name
        ...
        if (qsize < 20) {
            ...
            taskQueues.get(queueId).offer(new Runnable() {
                public void run() {
                    ...
                    final PowerManager.WakeLock wl = HelperClass.getWakeLock(queueId, 3600000);
                    try {
                        ...
                        next(queueId);
                        HelperClass.releaseWakeLock(wl); // will stack wakelocks
        ...
        if (currentTask.get(queueId) == null) {
            ...
            final String err = HelperClass.hourMinuteString() + " Task deadlock on: " + queueId + "! @" + qsize;
            Log.e(TAG, err);
