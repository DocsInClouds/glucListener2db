...
public class Pref {
    ...
    private static void initializePrefs() {
        if (prefs == null) {
            if (BackgroundClass.getAppContext() != null) {
                prefs = PreferenceManager.getDefaultSharedPreferences(BackgroundClass.getAppContext());
                if ((prefs == null) && (HelperClass.ratelimit("prefs-failure1", 20))) {
                    Log.wtf(TAG, "Could not initialize preferences due to init failure!!");
                }
            } else {
                if (HelperClass.ratelimit("prefs-failure2", 20)) {
                    Log.wtf(TAG, "Could not initialize preferences due to missing context!!");
                }
            }
        }
    }
    ...
    public static double getStringToDouble(final String pref, final double defaultValue) {
        try {
            return HelperClass.tolerantParseDouble(getString(pref, Double.toString(defaultValue)), defaultValue);
        } catch (Exception e) {
            return defaultValue;
        }
    }
