...
//                        }
            ...
            if (lastTransmitterData != null && lastTransmitterData.timestamp >= transmitterData.timestamp) {
                Log.e(TAG, "Rejecting TransmitterData constraint: last: " + HelperClass.dateTimeText(lastTransmitterData.timestamp) + " >= this: " + HelperClass.dateTimeText(transmitterData.timestamp));
                return null;
            }
            if (lastTransmitterData != null && lastTransmitterData.raw_data == transmitterData.raw_data && Math.abs(lastTransmitterData.timestamp - transmitterData.timestamp) < (
                Constants.MINUTE_IN_MS * 2)) {
                Log.e(TAG, "Rejecting identical TransmitterData constraint: last: " + HelperClass.dateTimeText(lastTransmitterData.timestamp) + " due to 2 minute rule this: " + HelperClass.dateTimeText(transmitterData.timestamp));
                return null;
            }
            //DIC: removed because not needed?
...
//                Log.e(TAG, "Rejecting historical TransmitterData constraint: calib: " + HelperClass.dateTimeText(lastCalibration.timestamp) + " > this: " + HelperClass.dateTimeText(transmitterData.timestamp));
//                return null;
//            }
            ...
            Log.e(TAG, "Got exception processing fields in protocol: " + e );//DIC+ " " + HexDump.dumpHexString(buffer));
        }
        return null;
    }
    ...
    public static void updateTransmitterBatteryFromSync(final int battery_level) {
        try {
            ...
            {
                td=TransmitterData.create(0,battery_level,(long)HelperClass.ts());
                Log.d(TAG,"Created new fake transmitter data record for battery sync");
                if (td==null) return;
            }
            if ((battery_level != td.sensor_battery_level) || ((HelperClass.ts()-td.timestamp)>(1000*60*60))) {
                td.sensor_battery_level = battery_level;
                td.timestamp = (long)HelperClass.ts(); // freshen timestamp on this bogus record for system status
    ...
    private static double roundRaw(TransmitterData td) {
        return HelperClass.roundDouble(td.raw_data,3);
    }
    private static double roundFiltered(TransmitterData td) {
        return HelperClass.roundDouble(td.filtered_data,3);
    }
