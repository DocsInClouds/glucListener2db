...
import com.docsinclouds.glucose2db.G5Utils.BestGZIPOutputStream;
import com.docsinclouds.glucose2db.G5Utils.CipherUtils;
import com.docsinclouds.glucose2db.G5Utils.Pref;
import com.docsinclouds.glucose2db.G5Utils.XdripNotification;
import com.docsinclouds.glucose2db.G5Utils.XdripNotificationCompat;
...
public class HelperClass {
    ...
    public static String base64encode(String input) {
        return new String(Base64.encode(input.getBytes(StandardCharsets.UTF_8), Base64.NO_WRAP),
            StandardCharsets.UTF_8);
    }

    public static String base64decode(String input) {
        try {
            return new String(Base64.decode(input.getBytes(StandardCharsets.UTF_8), Base64.NO_WRAP),
                StandardCharsets.UTF_8);
    ...
    public static String base64encodeBytes(byte[] input) {
        return new String(Base64.encode(input, Base64.NO_WRAP), StandardCharsets.UTF_8);
    }
    ...
    public static void persistentBuggySamsungCheck() {
        if (!buggy_samsung) {
            if (HelperClass.isSamsung() && PersistentStore.getLong(BUGGY_SAMSUNG_ENABLED) > 4) {
                buggy_samsung = true;
                Log.d(TAG,"Enabling buggy samsung mode due to historical pattern");
            }
        }
    }
    ...
    public static synchronized boolean ratelimitmilli(String name, int milliseconds) {
        // check if over limit
        if ((rateLimits.containsKey(name)) && (HelperClass.tsl() - rateLimits.get(name) < (milliseconds))) {
    ...
    public static String getDeviceDetails() {
        ...
        final String version =
            Build.VERSION.SDK_INT + " " + Build.VERSION.RELEASE + " " + Build.VERSION.INCREMENTAL;
        return manufacturer + " " + model + " " + version;
    }

    public static String getVersionDetails() {
        try {
            return BackgroundClass.getAppContext().getPackageManager().getPackageInfo(BackgroundClass.getAppContext().getPackageName(), PackageManager.GET_META_DATA).versionName;
        } catch (Exception e) {
    ...
    public static boolean getWifiSleepPolicyNever() {
        try {
            int policy = Settings.Global.getInt(BackgroundClass.getAppContext().getContentResolver(), android.provider.Settings.Global.WIFI_SLEEP_POLICY);
    ...
    public static void fixActionBar(AppCompatActivity context) {
        try {
            ...
            context.getSupportActionBar().setIcon(R.drawable.dic_logo);
        } catch (Exception e) {
    ...
    public static String niceTimeScalar(long t) {
        String unit = BackgroundClass.getAppContext().getString(R.string.unit_second);
        t = t / 1000;
        if (t != 1) unit = BackgroundClass.getAppContext().getString(R.string.unit_seconds);
        if (t > 59) {
            unit = BackgroundClass.getAppContext().getString(R.string.unit_minute);
            t = t / 60;
            if (t != 1) unit = BackgroundClass.getAppContext().getString(R.string.unit_minutes);
            if (t > 59) {
                unit = BackgroundClass.getAppContext().getString(R.string.unit_hour);
                t = t / 60;
                if (t != 1) unit = BackgroundClass.getAppContext().getString(R.string.unit_hours);
                if (t > 24) {
                    unit = BackgroundClass.getAppContext().getString(R.string.unit_day);
                    t = t / 24;
                    if (t != 1) unit = BackgroundClass.getAppContext().getString(R.string.unit_days);
                    if (t > 28) {
                        unit = BackgroundClass.getAppContext().getString(R.string.unit_week);
                        t = t / 7;
                        if (t != 1) unit = BackgroundClass.getAppContext().getString(R.string.unit_weeks);
                    }
                }
            }
        }
    ...
    public static String niceTimeScalarNatural(long t) {
        ...
        if ((t > Constants.DAY_IN_MS) && (t < Constants.WEEK_IN_MS * 2)) {
            ...
            final String day = df.format(new Date(HelperClass.tsl() + t));
    ...
    public static void fullDatabaseReset() {
        try {
            ...
            ActiveAndroid.initialize(BackgroundClass.getAppContext());
        } catch (Exception e) {
    ...
    public static boolean isLANConnected() {
        final ConnectivityManager cm =
            (ConnectivityManager) BackgroundClass.getAppContext().getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static String getWifiSSID() {
        try {
            final WifiManager wifi_manager = (WifiManager) BackgroundClass.getAppContext().getApplicationContext().getSystemService(Context.WIFI_SERVICE);
    ...
    public static boolean getWifiFuzzyMatch(String local, String remote) {
        ...
        //Log.d(TAG, "l:" + local + " r:" + remote + " slen:" + slen + " llen:" + llen + " matched:" + matched + "  q:" + HelperClass.qs(quota, 2) + "  dm:" + dmatch + " RESULT: " + result);
        return result;
    }
            ...
            if (!runOnUiThread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        //DIC //MainActivity.toaststatic(msg);
                    }
                }
            })) {
    ...
    public static void static_toast_long(final String msg) {
        static_toast(BackgroundClass.getAppContext(), msg, Toast.LENGTH_LONG);
    }

    public static void static_toast_short(final String msg) {
        static_toast(BackgroundClass.getAppContext(), msg, Toast.LENGTH_SHORT);
    }
    ...
    public static String getResourceURI(int id) {
        return ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + BackgroundClass.getAppContext().getPackageName() + "/" + id;
    }

    public static synchronized MediaPlayer playSoundUri(String soundUri) {
        try {
            HelperClass.getWakeLock("joh-playsound", 10000);
            final MediaPlayer player = MediaPlayer.create(BackgroundClass.getAppContext(), Uri.parse(soundUri));
    ...
    public static void stopService(Class c) {
        BackgroundClass.getAppContext().stopService(new Intent(BackgroundClass.getAppContext(), c));
    }

    public static void startService(Class c) {
        BackgroundClass.getAppContext().startService(new Intent(BackgroundClass.getAppContext(), c));
    }

    public static void startService(final Class c, final String... args) {
        final Intent intent = new Intent(BackgroundClass.getAppContext(), c);
        if (args.length % 2 == 1) {
            throw new RuntimeException("Odd number of args for HelperClass.startService");
        }
        for (int i = 0; i < args.length; i += 2) {
            intent.putExtra(args[i], args[i + 1]);
        }
        BackgroundClass.getAppContext().startService(intent);
    }


    public static void startActivity(Class c) {
        BackgroundClass.getAppContext().startActivity(getStartActivityIntent(c));
    }


    public static Intent getStartActivityIntent(Class c) {
        return new Intent(BackgroundClass.getAppContext(), c).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    }
...
//DIC
    ...
    private static NotificationCompat.Builder notificationBuilder(String title, String content, PendingIntent intent, String channelId) {
        return new NotificationCompat.Builder(BackgroundClass.getAppContext(), channelId)
            //DIC .setSmallIcon( R.drawable.ic_action_communication_invert_colors_on)
                ...
                if ((mBluetoothDeviceAddress != null) && (device.getAddress().equals(mBluetoothDeviceAddress))) {
                    ...
                    try {
                        ...
                        if (type != PAIRING_VARIANT_PIN && type != PAIRING_VARIANT_PASSKEY) {
                            ...
                            HelperClass.static_toast_short("xDrip Pairing");
                            broadcastReceiver.abortBroadcast();
                        } else {
                        ...
                        if (HelperClass.ratelimit("failed pair confirmation", 200)) {
                            // BluetoothDevice.PAIRING_VARIANT_CONSENT)
                            if (type == 3) {
                                HelperClass.static_toast_long("Please confirm the bluetooth pairing request");
                                return false;
                            } else {
                                HelperClass.static_toast_long("Failed to pair, may need to do it via Android Settings");
    ...
    public static boolean refreshDeviceCache(String thisTAG, BluetoothGatt gatt){
        try {
            final Method method = gatt.getClass().getMethod("refresh");
            if (method != null) {
    ...
    public static synchronized void unBond(String transmitterMAC) {
        ...
        try {
            final BluetoothAdapter mBluetoothAdapter = ((BluetoothManager) BackgroundClass.getAppContext().getSystemService(Context.BLUETOOTH_SERVICE)).getAdapter();
            ...
            Log.e(TAG, "Exception during unbond! " + transmitterMAC, e);
        }
    ...
    public static boolean isServiceRunningInForeground(Class<?> serviceClass) {
        final ActivityManager manager = (ActivityManager) BackgroundClass.getAppContext().getSystemService(Context.ACTIVITY_SERVICE);
        try {
    ...
    public synchronized static Notification createOngoingNotificationWithReading(String reading) {
        Context mContext = BackgroundClass.getAppContext();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            b.setVisibility(
                Pref.getBooleanDefaultFalse("public_notifications") ? Notification.VISIBILITY_PUBLIC
                    : Notification.VISIBILITY_PRIVATE);
            b.setCategory(NotificationCompat.CATEGORY_STATUS);
        }
        b.setContentTitle(
            "last reading: " + ((reading != null && !reading.equals("")) ? reading : "--"))
            .setContentText("Dexcom collection service is running.")
            .setSmallIcon(R.mipmap.ic_launcher)
            .setLargeIcon(
                BitmapFactory
                    .decodeResource(BackgroundClass.getAppContext().getResources(),
                        R.mipmap.ic_launcher));
        b.setWhen(new Date().getTime()); //TODO DIC maybe use proper timestamp in future
      b.setContentIntent(
          PendingIntent.getActivity(mContext, 324, intent, PendingIntent.FLAG_UPDATE_CURRENT));
