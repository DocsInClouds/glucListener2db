...
import static com.docsinclouds.glucose2db.Constants.G5_CALIBRATION_REQUEST;
import static com.docsinclouds.glucose2db.Constants.G5_SENSOR_FAILED;
import static com.docsinclouds.glucose2db.Constants.G5_SENSOR_RESTARTED;
import static com.docsinclouds.glucose2db.Constants.G5_SENSOR_STARTED;
...
import static com.docsinclouds.glucose2db.G5Model.BluetoothServices.getUUIDName;
import static com.docsinclouds.glucose2db.G5Model.CalibrationState.Unknown;
import static com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine.CLOSED_OK_TEXT;
import static com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine.evaluateG6Settings;
import static com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine.pendingCalibration;
import static com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine.pendingStart;
import static com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine.pendingStop;
import static com.docsinclouds.glucose2db.G5Utils.DexCollectionType.DexcomG5;
import static com.docsinclouds.glucose2db.MainActivity.mGlucoseViewModel;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.BOND;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.CLOSE;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.CLOSED;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.CONNECT;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.CONNECT_NOW;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.DISCOVER;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.GET_DATA;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.INIT;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.PREBOND;
import static com.docsinclouds.glucose2db.Ob1G5CollectionService.STATE.SCAN;
...
import com.docsinclouds.glucose2db.G5Model.BluetoothServices;
import com.docsinclouds.glucose2db.G5Model.CalibrationState;
import com.docsinclouds.glucose2db.G5Model.DexSyncKeeper;
import com.docsinclouds.glucose2db.G5Model.Ob1G5StateMachine;
...
import com.docsinclouds.glucose2db.G5Utils.RxBleProvider;
...
public class Ob1G5CollectionService extends G5BaseService {
  ...
  public synchronized void background_automata(final int timeout) {
    ...
    final PowerManager.WakeLock wl = HelperClass.getWakeLock("jam-g5-background", timeout + 5000);
  ...
  private synchronized void automata() {
    ...
    if ((last_automata_state != state) || state == INIT || (HelperClass.ratelimit("jam-g5-dupe-auto", 2))) {
      ...
      try {
        switch (state) {
          ...
          case SCAN:
            ...
            if (HelperClass.msSince(static_last_connected) > 30 * 60 * 1000) {
              if (HelperClass.pratelimit("ob1-collector-restart", 1200)) {
            ...
            if (useMinimizeScanningStrategy()) {
              Log.d(TAG, "Skipping Scanning! : Changing state due to minimize_scanning flags");
...
//              msg("Pausing");
//              Log.d(TAG, "Pausing for alt: ");
//              HelperClass.threadSleep(1000);
//            }
            if (do_discovery) {
              discover_services();
            } else {
          ...
          case CHECK_AUTH:
            if (do_auth) {
              final PowerManager.WakeLock linger_wl_connect = HelperClass.getWakeLock("jam-g5-check-linger", 6000);
          ...
          case PREBOND:
            if (!getInitiateBondingFlag()) {
              final PowerManager.WakeLock linger_wl_prebond = HelperClass.getWakeLock("jam-g5-prebond-linger", 16000);
              if (!Ob1G5StateMachine.doKeepAliveAndBondRequest(this, connection))
                resetState();
            } else {
              Log.d(TAG, "Going directly to initiate bond");
              changeState(BOND);
            }
            break;
          case BOND:
            if (getInitiateBondingFlag()) {
              Log.d(TAG, "State bond attempting to create bond");
            } else {
              Log.d(TAG, "State bond currently does nothing as setting disabled");
            }
            create_bond();
            break;
          case RESET:
          ...
          case GET_DATA:
            if (hardResetTransmitterNow) {
              ...
              final PowerManager.WakeLock linger_wl_get_data = HelperClass.getWakeLock("jam-g5-get-linger", 6000);
  ...
  private boolean useMinimizeScanningStrategy() {
    ...
    Log.d(TAG, "minimize: " + minimize_scanning + " mac: " + transmitterMAC + " lastfailed:" + lastConnectFailed + " nowfail:" + connectNowFailures + " stimeout:" + scanTimeouts + " modulo:" + modulo);
...
//    }
    return minimize_scanning && transmitterMAC != null && (!lastConnectFailed || (modulo == 1) || alwaysMinimize) && (DexSyncKeeper
        .isReady(transmitterID));
  }

  //DIC
//  private void estimateAnticipateFromLinkedData() {
//    final BgReading bg = BgReading.last();
//    if (bg != null && bg.timestamp > static_last_connected && HelperClass.msSince(bg.timestamp) < HOUR_IN_MS * 3) {
//      final long ts = bg.timestamp - Constants.SECOND_IN_MS;
//      Log.d(TAG, "Updating Sync Keeper with network learned timestamp: " + HelperClass.dateTimeText(ts));
...
//  }
  ...
  private static void init_tx_id() {
    transmitterID = Pref.getString(BackgroundClass.getAppContext().getResources().getString(R.string.prefs_dexcomTransmitterId), "NULL");
  }
  ...
  private synchronized void scan_for_device() {
    if (state == SCAN) {
      ...
      if (always_scan || scan_next_run || (transmitterMAC == null) || (!transmitterID.equals(transmitterIDmatchingMAC)) || (static_last_timestamp < 1)) {
        ...
        scanWakeLock = HelperClass.getWakeLock("xdrip-jam-g5-scan", (int) Constants.MINUTE_IN_MS * 7);
        ...
        if (minimize_scanning) {
          ...
          Inevitable.task(STOP_SCAN_TASK_ID, 320 * SECOND_IN_MS, this::stopScanWithTimeoutAndReschedule);
        }
  ...
  private void stopScanWithTimeoutAndReschedule() {
    stopScan();
    Log.d(TAG, "Stopped scan due to timeout at: " + HelperClass.dateTimeText(HelperClass.tsl()));
    scanTimeouts++;
    tryLoadingSavedMAC();
    prepareToWakeup();
  }
    ...
    if ((state == CONNECT) || (state == CONNECT_NOW)) {
      ...
      if (localTransmitterMAC != null) {
        ...
        if (state == CONNECT_NOW) {
          if (connection_linger != null) HelperClass.releaseWakeLock(connection_linger);
        ...
        try {
          ...
          last_connect_started = HelperClass.tsl();
          ...
          Log.e(TAG, "Caught IllegalArgument Exception: " + e + " retry on next run");
  ...
  public synchronized void reset_bond(boolean allow) {
    if (allow || (HelperClass.pratelimit("ob1-bond-cycle", 7200))) {
      Log.e(TAG, "Attempting to refresh bond state");
      msg("Resetting Bond");
      unBond();
      do_create_bond();
    }
  ...
  private synchronized void do_create_bond() {
    ...
    Log.d(TAG, "Attempting to create bond, device is : " + (isDeviceLocallyBonded ? "BONDED" : "NOT Bonded"));

    if (isDeviceLocallyBonded && getInitiateBondingFlag()) {
      Log.d(TAG, "Device is marked as bonded but we are being asked to bond so attempting to unbond first");
      unbondIfAllowed();
      changeState(CLOSE);
    } else {
  ...
  private void tryLoadingSavedMAC() {
    if ((transmitterMAC == null) || (!transmitterIDmatchingMAC.equals(transmitterID))) {
      ...
      Log.d(TAG, "MAC for transmitter id already populated: " + transmitterID + " " + transmitterMAC);
    }
  }
...
//        if (HelperClass.quietratelimit("forced-wear-notice", 3))
//          Log.d(TAG, "Not running due to forced wear");
//        return false;
//      }
    } else {
      // android wear code
  ...
  private void handleWakeup() {
    if (always_scan) {
      ...
      if (connectFailures > 0 || (!use_auto_connect && connectNowFailures > 0)) {
        always_scan = true;
        Log.e(TAG, "Switching to scan always mode due to connect failures metric: " + connectFailures);
        changeState(SCAN);
      } else if (use_auto_connect && (connectNowFailures > 1) && (connectFailures < 0)) {
        Log.d(TAG, "Avoiding power connect due to failure metric: " + connectNowFailures + " " + connectFailures);
        changeState(CONNECT);
      } else {
        changeState(CONNECT_NOW);
      }
  ...
  private synchronized void prepareToWakeup() {
    if (HelperClass.ratelimit("g5-wakeup-timer", 5)) {
      ...
      if (when > 0) {
        final long when_offset = when - HelperClass.tsl();
        Log.d(TAG, "(" + HelperClass.dateTimeText(HelperClass.tsl()) + ")  Wake up time anticipated at: " + HelperClass.dateTimeText(when));
        scheduleWakeUp(when_offset - SECOND_IN_MS * 15, "anticipate");
      } else {
    ...
    if ((android_wear && wakeup_jitter > TOLERABLE_JITTER) || always_connect) {
      ...
      Log.d(TAG, "Not stopping connect due to " + (always_connect ? "always_connect flag" : "unreliable wake up"));
      state = CONNECT;
  ...
  private void scheduleWakeUp(long future, final String info) {
    ...
    Log.d(TAG, "Scheduling wakeup @ " + HelperClass.dateTimeText(HelperClass.tsl() + future) + " (" + info + ")");
  ...
  private void checkAlwaysScanModels() {
    ...
    if ((HelperClass.isSamsung() && PersistentStore.getLong(BUGGY_SAMSUNG_ENABLED) > 4)) {
      Log.d(TAG, "Enabling buggy samsung due to persistent metric");
  ...
  public void onCreate() {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
      ...
      try {
        ...
        Log.e(TAG, "Could not register bond state receiver: " + e);
      }
      ...
      try {
        ...
        Log.e(TAG, "Could not register pairing request receiver:" + e);
      }
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    MainActivity.checkAppContext(getApplicationContext());
    final PowerManager.WakeLock wl = HelperClass.getWakeLock("g5-start-service", 310000);
    try {
      Log.d(TAG, "WAKE UP WAKE UP WAKE UP WAKE UP @ " + HelperClass.dateTimeText(HelperClass.tsl()));
      msg("Wake up");
      if (wakeup_time > 0) {
        ...
        if (wakeup_jitter < 0) {
          ...
          if (wakeup_jitter > 1000) {
            ...
            if ((wakeup_jitter > TOLERABLE_JITTER) && (!HelperClass.buggy_samsung) && HelperClass.isSamsung()) {
              Log.wtf(TAG, "Enabled Buggy Samsung workaround due to jitter of: " + HelperClass.niceTimeScalar(wakeup_jitter));
              HelperClass.buggy_samsung = true;
      ...
      if (HelperClass.quietratelimit("evaluateG6Settings", 600)) {
        evaluateG6Settings();
      }
  ...
  private synchronized void stopScan() {
    ...
    Log.d(TAG, "DEBUG: killing stop scan task");
  ...
  private boolean isScanMatch(final String this_address, final String historical_address, final String this_name, final String search_name) {
    ...
    if (result) {
      if (historical_address.length() == this_address.length()
        ...
        if (HelperClass.ratelimit("ob1-address-change-error", 30)) {
          Log.wtf(TAG, "Bluetooth device: " + search_name + " apparently changed from mac: " + historical_address + " to: " + this_address + " :: There appears to be confusion between devices - ignoring this scan result");
        }
        result = false;
      }
    }
  ...
  private synchronized void onScanResult(ScanResult bleScanResult) {
    ...
    if (isScanMatch(this_address, historicalTransmitterMAC, this_name, search_name)) {
      ...
      Log.d(TAG, "Got scan result match: " + bleScanResult.getBleDevice().getName() + " " + this_address + " rssi: " + this_rssi);
      ...
      if (HelperClass.quietratelimit("bt-obi1-null-match" + this_mac, 15)) {
        Log.d(TAG, "Bluetooth scanned device doesn't match (" + search_name + ") found: " + this_name + " " + bleScanResult.getBleDevice().getMacAddress());
      }
    }
  }
  ...
  private synchronized void onScanFailure(Throwable throwable) {
    ...
    if (throwable instanceof BleScanException) {
      ...
      if (((BleScanException) throwable).getReason() == BleScanException.SCAN_FAILED_APPLICATION_REGISTRATION_FAILED) {
        if (HelperClass.ratelimit("bluetooth-cannot-register", 120)) {
          if (Pref.getBooleanDefaultFalse("automatically_turn_bluetooth_on")) {
            Log.wtf(TAG, "Android bluetooth appears broken - attempting to turn off and on");
            HelperClass.niceRestartBluetooth(BackgroundClass.getAppContext());
          } else {
            Log.e(TAG, "Cannot reset bluetooth due to preference being disabled");
          }
        }
      }
...
//DIC
//          if (WholeHouse.isRpi()) {
//            Log.e(TAG,"Trying to turn off/on bluetooth");
//            HelperClass.niceRestartBluetooth(BackgroundClass.getAppContext());
//          } else {
...
//          }
      ...
      if (minimize_scanning) {
        Log.d(TAG, "Preparing to wake up at next expected time");
        prepareToWakeup();
      } else {
        backoff_automata();
      }
  ...
  private void unbondIfAllowed() {
    if (Pref.getBoolean("ob1_g5_allow_resetbond", true)) {
      ...
      Log.e(TAG, "Would have tried to unpair but preference setting prevents it. (unbond)");
    }
  }
  ...
  private boolean resetBondIfAllowed(boolean force) {
    if (Pref.getBoolean("ob1_g5_allow_resetbond", true)) {
      ...
      Log.e(TAG, "Would have tried to unpair but preference setting prevents it. (resetbond)");
      return false;
    }
  }
  ...
  private void onConnectionFailure(final Throwable throwable) {
    ...
    if (state == DISCOVER) {
      ...
      if (!resetBondIfAllowed(false) && android_wear) {
        Log.d(TAG, "Trying alternate reconnection strategy");
        changeState(CONNECT_NOW);
      }
      return;
    }
    ...
    if (state == CONNECT_NOW) {
      ...
      if (throwable instanceof BleGattCallbackTimeoutException) {
        if (throwable.getMessage().contains("BleGattOperation{description='CONNECTION_STATE'")) {
          Log.d(TAG, "Setting pre-scan failure marker enabled due to exception type");
          preScanFailureMarker = true;
        }
      } else if (HelperClass.msSince(last_connect_started) < SECOND_IN_MS) {
        Log.d(TAG, "Setting pre-scan failure marker enabled due to exception timing");
        preScanFailureMarker = true;
      }
  ...
  private synchronized void onConnectionStateChange(RxBleConnection.RxBleConnectionState newState) {
    ...
    switch (newState) {
      ...
      case CONNECTED:
        ...
        if ((connecting_time > static_last_timestamp) && (since_connecting > SECOND_IN_MS * 310) && (since_connecting < SECOND_IN_MS * 620)) {
          if (!always_scan) {
            Log.e(TAG, "Connection time shows missed reading, switching to always scan, metric: " + HelperClass.niceTimeScalar(since_connecting));
            always_scan = true;
          } else {
            Log.e(TAG, "Connection time shows missed reading, despite always scan, metric: " + HelperClass.niceTimeScalar(since_connecting));
          }
        }
        break;
      case DISCONNECTING:
  ...
  private void onServicesDiscovered(RxBleDeviceServices services) {
    for (BluetoothGattService service : services.getBluetoothGattServices()) {
      if (d) Log.d(TAG, "Service: " + getUUIDName(service.getUuid()));
      if (service.getUuid().equals(BluetoothServices.CGMService)) {
        if (d) Log.i(TAG, "Found CGM Service!");
        if (!always_discover) {
          do_discovery = false;
        }
        changeState(STATE.CHECK_AUTH);
...
//DIC        if (!android_wear) HelperClass.playResourceAudio(R.raw.labbed_musical_chime);
      }
    }
  ...
  private static class GattRefreshOperation implements RxBleCustomOperation<Void> {
    ...
    private Void refreshDeviceCache(final BluetoothGatt gatt) {
      Log.d(TAG, "Gatt Refresh " + (HelperClass.refreshDeviceCache(TAG, gatt) ? "succeeded" : "failed"));
      return null;
    }
  }
  ...
  public void instantCreateBondIfAllowed() {
    if (getInitiateBondingFlag()) {
      ...
      Log.e(TAG, "instantCreateBond blocked by lack of initiate_bonding flag");
    }
  }
  ...
  private final BroadcastReceiver mPairingRequestRecevier = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if ((bleDevice != null) && (bleDevice.getBluetoothDevice().getAddress() != null)) {
        ...
        //TODO DIC: it this needed? if (!android_wear) Home.startHomeWithExtra(context, Home.HOME_FULL_WAKEUP, "1");
        if (!HelperClass.doPairingRequest(context, this, intent, bleDevice.getBluetoothDevice().getAddress())) {
          if (!android_wear) {
            unregisterPairingReceiver();
            Log.e(TAG, "Pairing failed so removing pairing automation"); // todo use flag
          }
        }
      } else {
  ...
  public static void processCalibrationState(CalibrationState state) {
    ...
    if (state == Unknown) {
      Log.d(TAG, "Not processing push of unknown state as this is the unset state");
      return;
    }
...
//        final PendingIntent pi = PendingIntent.getActivity(BackgroundClass.getAppContext(), G5_CALIBRATION_REQUEST, HelperClass.getStartActivityIntent(c), PendingIntent.FLAG_UPDATE_CURRENT);
//        // pending intent not used on wear
//        HelperClass.showNotification(state.getText(), "G5 Calibration Required", android_wear ? null : pi, G5_CALIBRATION_REQUEST, state == CalibrationState.NeedsFirstCalibration, true, false);
//
//      });
      Log.e(TAG,"DIC: needs calibration - not implemented");
    } else if (!needs_calibration && was_needing_calibration) {
      HelperClass.cancelNotification(G5_CALIBRATION_REQUEST);
    }
    ...
    if (!is_started && was_started) {
      if (Pref.getBooleanDefaultFalse("ob1_g5_restart_sensor") && (Sensor.isActive())) {
        if (state.ended()) {
          Log.d(TAG, "Requesting time-travel restart");
          Ob1G5StateMachine.restartSensorWithTimeTravel();
        } else {
          Log.d(TAG, "Attempting to auto-start sensor");
          Ob1G5StateMachine.startSensor(HelperClass.tsl());
        }
        final PendingIntent pi = PendingIntent.getActivity(BackgroundClass.getAppContext(), G5_SENSOR_RESTARTED, HelperClass.getStartActivityIntent(MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
        HelperClass.showNotification("Auto Start", "Sensor Requesting Restart", pi, G5_SENSOR_RESTARTED, true, true, false);
      }
      final PendingIntent pi = PendingIntent.getActivity(BackgroundClass.getAppContext(), G5_SENSOR_STARTED, HelperClass.getStartActivityIntent(MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
      HelperClass.showNotification(state.getText(), "Sensor Stopped", pi, G5_SENSOR_STARTED, true, true, false);
      Log.d(TAG, "Native Sensor is now Stopped: " + state.getExtendedText());
    } else if (is_started && !was_started) {
      HelperClass.cancelNotification(G5_SENSOR_STARTED);
      Log.d(TAG, "Native Sensor is now Started: " + state.getExtendedText());
    }

    if (is_failed && !was_failed) {
      final PendingIntent pi = PendingIntent.getActivity(BackgroundClass.getAppContext(), G5_SENSOR_FAILED, HelperClass.getStartActivityIntent(MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
      HelperClass.showNotification(state.getText(), "G5 Sensor FAILED", pi, G5_SENSOR_FAILED, true, true, false);
      Log.d(TAG, "Native Sensor is now marked FAILED: " + state.getExtendedText());
    }
  ...
  private static void storeCalibrationState(final CalibrationState state) {
    ...
    PersistentStore.setLong(OB1G5_STATESTORE_TIME, HelperClass.tsl());
  }

  private static CalibrationState getStoredCalibrationState() {
    if (HelperClass.msSince(PersistentStore.getLong(OB1G5_STATESTORE_TIME)) < HOUR_IN_MS * 2) {
  ...
  private static void loadCalibrationStateAsRequired() {
    if ((lastSensorState == null) && HelperClass.quietratelimit("ob1-load-sensor-state", 5)) {
  ...
  public static boolean isG5ActiveButUnknownState() {
    ...
    return (lastSensorState == null || lastSensorState == Unknown)
        && usingNativeMode();
  }
...
*/
  ...
  public static List<StatusItem> megaStatus() {
    ...
    l.add(new StatusItem("Phone Service State", lastState, HelperClass.msSince(lastStateUpdated) < 300000 ? (lastState.startsWith("Got data") ? Highlight.GOOD : NORMAL) : (isWatchRunning() ? Highlight.GOOD : CRITICAL)));
    if (last_scan_started > 0) {
      final long scanning_time = HelperClass.msSince(last_scan_started);
      l.add(new StatusItem("Time scanning", HelperClass.niceTimeScalar(scanning_time), scanning_time > Constants.MINUTE_IN_MS * 5 ? (scanning_time > Constants.MINUTE_IN_MS * 10 ? BAD : NOTICE) : NORMAL));
    }
    ...
    if (static_last_connected > 0) {
      l.add(new StatusItem("Last Connected", HelperClass.niceTimeScalar(HelperClass.msSince(static_last_connected)) + " ago"));
    }
    ...
    if (lastUsableGlucosePacketTime != 0) {
      if (HelperClass.msSince(lastUsableGlucosePacketTime) < Constants.MINUTE_IN_MS * 15) {
        l.add(new StatusItem("G5 Algorithm", "Data Received " + HelperClass.hourMinuteString(lastUsableGlucosePacketTime), Highlight.GOOD));
      }
    }
    ...
    if (max_wakeup_jitter > 5000) {
      l.add(new StatusItem("Slowest Wakeup ", HelperClass.niceTimeScalar(max_wakeup_jitter), max_wakeup_jitter > SECOND_IN_MS * 10 ? CRITICAL : NOTICE));
    }

    if (HelperClass.buggy_samsung) {
    ...
    if (Pref.getBooleanDefaultFalse("wear_sync") &&
      ...
      if (static_last_timestamp_watch > 0) {
        l.add(new StatusItem("Watch got Glucose", HelperClass.niceTimeSince(static_last_timestamp_watch) + " ago"));
      }
    }
    ...
    if (HelperClass.quietratelimit("update-g5-battery-warning", 10)) {
      updateBatteryWarningLevel();
    }

    if ((bt != null) && (last_battery_query > 0)) {
      l.add(new StatusItem("Battery Last queried", HelperClass.niceTimeSince(last_battery_query) + " " + "ago", NORMAL, "long-press",
          new Runnable() {
            @Override
            public void run() {
              getBatteryStatusNow = true;
      ...
      l.add(new StatusItem("Transmitter Days", ((bt.runtime > -1) ? bt.runtime : "") + ((last_transmitter_timestamp > 0) ? " / " + HelperClass.qs((double) last_transmitter_timestamp / 86400, 1) : "")));
  ...
  public static void resetSomeInternalState() {
    Log.d(TAG, "Resetting internal state by request");
  ...
  public void listenForChangeInSettings(boolean listen) {
    try {
      ...
      Log.e(TAG, "Error with preference listener: " + e + " " + listen);
    }
  }
  ...
  public static boolean isCollecting() {
    return (state == CONNECT_NOW && HelperClass.msSince(static_last_timestamp) < Constants.MINUTE_IN_MS * 30) || HelperClass.msSince(static_last_timestamp) < Constants.MINUTE_IN_MS * 6;
  }
  ...
  /**
   * saves one new value in db for plotter class timestamp is generated
   */
  public static void saveNewGlucoValueInDB(int value, double trend, long timestamp) {
    GlucoEntity gE = new GlucoEntity(timestamp, value, trend);
    mGlucoseViewModel.insert(gE);
    Log.d(TAG, "inserted to db " + gE.getGlucoValue());
  }
}
